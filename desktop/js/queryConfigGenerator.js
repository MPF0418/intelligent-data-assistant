...
class QueryConfigGenerator {
    constructor() {
        ...
        this.configGenerators = {
            ...
            'CHART_BAR': {
                patterns: [
                    ...
                    {
                        // V4.0紧急修复：按照X绘制Y柱状图（没有"的"字）
                        // 如"按照地区绘制销售额柱状图" → X轴=地区, Y轴=销售额
                        regex: /按[照]?(.+?)(绘制|画|生成|做)(.+?)(柱状图|条形图)$/,
                        extract: (match, columns, userInput) => {
                            const xDesc = match[1].trim();
                            const yDesc = match[3].trim();
                            console.log(`[CHART_BAR] 匹配"按照X绘制Y柱状图": X=${xDesc}, Y=${yDesc}`);
            ...
            'CHART_LINE': {
                patterns: [
                    ...
                    {
                        // V4.0紧急修复：按照X绘制Y折线图（没有"的"字）
                        regex: /按[照]?(.+?)(绘制|画|生成|做)(.+?)(折线图|趋势图)$/,
                        extract: (match, columns, userInput) => {
                            const xDesc = match[1].trim();
                            const yDesc = match[3].trim();
                            console.log(`[CHART_LINE] 匹配"按照X绘制Y折线图": X=${xDesc}, Y=${yDesc}`);
            ...
            'CHART_PIE': {
                patterns: [
                    ...
                    {
                        // V4.0紧急修复：按照X绘制Y饼图（没有"的"字）
                        regex: /按[照]?(.+?)(绘制|画|生成|做)(.+?)(饼图|占比图)$/,
                        extract: (match, columns, userInput) => {
                            const labelDesc = match[1].trim();
                            const valueDesc = match[3].trim();
                            console.log(`[CHART_PIE] 匹配"按照X绘制Y饼图": 标签=${labelDesc}, 数值=${valueDesc}`);
                            ...
                            const aggregateFunction = this.determineAggregateFunction(userInput, valueCol);
        ...
        // V4.0新增：分析要素API配置
        this.analysisApiUrl = 'http://localhost:5002';
        this.analysisCache = new Map();
    }
    
    // V4.0新增：调用分析要素API识别聚合函数
    async detectAggregateFunction(userInput) {
        if (!userInput) {
            return { function: 'sum', confidence: 0.5, method: 'default' };
        }
        ...
        try {
            const response = await fetch(`${this.analysisApiUrl}/api/analyze-elements`, {
                method: 'POST',
            ...
            const aggFunc = result.aggregate_function || 'sum';
            const confidence = result.aggregate_confidence || 0.5;
            
            const analysisResult = {
                function: aggFunc,
                confidence: confidence,
                method: 'analysis_model',
                probabilities: result.aggregate_probabilities
            };
            
            this.analysisCache.set(cacheKey, analysisResult);
            console.log(`[V4.0] 分析要素识别: 聚合函数=${aggFunc}, 置信度=${confidence.toFixed(2)}`);
            
            return analysisResult;
        } catch (error) {
            console.warn('[V4.0] 分析要素API调用失败，使用规则匹配:', error);
            return this.detectAggregateByRules(userInput);
        }
    }
    
    // V4.0新增：基于规则的聚合函数识别（兜底方案）
    detectAggregateByRules(userInput) {
        if (!userInput) {
            return { function: 'sum', confidence: 0.5, method: 'rule_default' };
        }
        
        const lowerInput = userInput.toLowerCase();
        
        const aggregateKeywords = {
            'sum': ['总和', '合计', '总计', '求和', '一共', '总共', '累加', '汇总'],
            'avg': ['平均', '均值', '人均', '平均值'],
            'max': ['最大', '最高', '最多', '峰值', '榜首'],
            'min': ['最小', '最低', '最少', '谷值'],
            'count': ['数量', '个数', '计数', '多少个', '频次', '次数'],
            'median': ['中位数', '中间值'],
            'std': ['标准差', '波动', '离散'],
            'ratio': ['占比', '百分比', '比例', '份额'],
            'growth_rate': ['增长率', '增幅', '涨幅'],
            'yoy': ['同比', '与去年同期'],
            'mom': ['环比', '与上月']
        };
        
        for (const [func, keywords] of Object.entries(aggregateKeywords)) {
            for (const keyword of keywords) {
                if (lowerInput.includes(keyword)) {
                    return { function: func, confidence: 0.8, method: 'rule' };
                }
            }
        }
    ...
    // V4.0新增：同步版本的聚合函数识别（用于patterns中调用）
    // 参数：userInput - 用户输入, yAxisColumn - Y轴列名（可选）
    determineAggregateFunction(userInput, yAxisColumn) {
        if (!userInput) {
            return 'sum';
        }
        ...
        for (const [func, keywords] of Object.entries(aggregateKeywords)) {
            for (const keyword of keywords) {
                if (lowerInput.includes(keyword)) {
                    console.log(`[determineAggregateFunction] 关键词匹配: "${keyword}" -> ${func}`);
                    return func;
                }
            }
        }
        ...
        // 2. 如果有Y轴列名，根据列名推断
        if (yAxisColumn) {
            const lowerY = yAxisColumn.toLowerCase();
            // 数量类列默认计数
            if (lowerY.includes('数量') || lowerY.includes('个数') || lowerY.includes('次数') || lowerY.includes('人数')) {
                return 'count';
            }
            // 金额类列默认求和
            if (lowerY.includes('销售额') || lowerY.includes('金额') || lowerY.includes('收入') || lowerY.includes('利润')) {
                return 'sum';
            }
            // 平均类列
            if (lowerY.includes('平均') || lowerY.includes('均值')) {
                return 'avg';
            }
        }
    ...
    // V4.0新增：检测输出目标类型
    async detectOutputType(userInput) {
        if (!userInput) {
            return { type: 'value', confidence: 0.5, method: 'default' };
        }
        ...
        try {
            ...
            const result = await response.json();
            const outputType = result.output_type || 'value';
            const confidence = result.output_confidence || 0.5;
            ...
            console.warn('[V4.0] 输出目标API调用失败，使用规则匹配:', error);
            return this.detectOutputTypeByRules(userInput);
        }
    }
    
    // V4.0新增：基于规则的输出目标识别（兜底方案）
    detectOutputTypeByRules(userInput) {
        if (!userInput) {
            return { type: 'value', confidence: 0.5, method: 'rule_default' };
        }
        
        const lowerInput = userInput.toLowerCase();
        
        const outputKeywords = {
            'chart_bar': ['柱状图', '条形图', '柱图'],
            'chart_line': ['折线图', '线图', '趋势图', '走势图'],
            'chart_pie': ['饼图', '占比图', '环形图'],
            'chart_scatter': ['散点图', '点图'],
            'chart_radar': ['雷达图', '蛛网图'],
            'chart_area': ['面积图', '区域图'],
            'table': ['表格', '列表', '清单', '明细', '一览表'],
            'value': ['是多少', '的值', '数值', '结果']
        };
        
        for (const [type, keywords] of Object.entries(outputKeywords)) {
            for (const keyword of keywords) {
                if (lowerInput.includes(keyword)) {
                    return { type: type, confidence: 0.8, method: 'rule' };
                }
            }
        }
    ...
    // V4.0新增：异步生成配置（集成分析要素识别）
    async generateConfigAsync(intentType, userInput, columns, entityExtractionResult = null) {
        console.log(`[V4.0] 异步生成配置: ${intentType}, 输入: "${userInput}"`);
        
        // 首先使用同步方法生成基础配置
        const baseConfig = this.generateConfig(intentType, userInput, columns, entityExtractionResult);
        
        if (!baseConfig) {
            return null;
        }
        
        // 如果是图表或聚合查询，尝试使用分析要素API识别聚合函数
        if (intentType.startsWith('CHART_') || intentType === 'QUERY_AGGREGATE') {
            try {
                const aggResult = await this.detectAggregateFunction(userInput);
                
                if (aggResult && aggResult.confidence > 0.6) {
                    // 使用模型识别的聚合函数
                    baseConfig.aggregateFunction = aggResult.function;
                    baseConfig.aggregateFunctionSource = aggResult.method;
                    baseConfig.aggregateFunctionConfidence = aggResult.confidence;
                    console.log(`[V4.0] 使用分析模型识别的聚合函数: ${aggResult.function} (${aggResult.confidence.toFixed(2)})`);
                }
            } catch (error) {
                console.warn('[V4.0] 分析要素识别失败，使用默认配置:', error);
            }
        }
